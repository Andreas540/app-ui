...
export default function TenantAdminCustomerSettingsTab() {
  ...
  return (
    <div>
      {/* Show/Hide customers section */}
      <div>
        <div style={sectionHeaderStyle} onClick={() => setExpanded(v => !v)}>
          <span>{t('tenantAdmin.customerSettings.showHideTitle')}</span>
          <span style={{ fontSize: 20, fontWeight: 'bold' }}>{expanded ? '−' : '+'}</span>
        </div>

        {expanded && (
          <div style={{ marginTop: 12 }}>
            <p style={{ marginTop: 0, marginBottom: 12, fontSize: 14, color: 'var(--text-secondary)', lineHeight: 1.6 }}>
            ...
            {customers.length === 0 ? (
              ...
              <>
                ...
                {/* Footer: hidden count + save */}
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
                  {hiddenCount > 0 ? (
                    <span style={{ fontSize: 13, color: 'var(--muted)' }}>
                      {t('tenantAdmin.customerSettings.hiddenCount', { count: hiddenCount })}
                    </span>
                  ) : <span />}
                  <button
                    className="primary"
