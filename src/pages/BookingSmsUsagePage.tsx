...
export default function BookingSmsUsagePage() {
  ...
  return (
    <div style={{ maxWidth: 800, margin: '0 auto', padding: '24px 16px' }}>
      <h2 style={{ marginBottom: 24 }}>SMS Usage</h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 28 }}>
          <div className="card" style={{ padding: 20, gridColumn: '1 / -1' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
              <div>
                <div className="helper">This month's SMS</div>
                <div style={{ fontSize: 28, fontWeight: 700 }}>{monthStats.billable_count}</div>
              </div>
              <div style={{ textAlign: 'right' }}>
                <div className="helper">Estimated cost</div>
                <div style={{ fontSize: 22, fontWeight: 700 }}>{fmtCurrency(monthStats.estimated_cost)}</div>
                <div className="helper">of {fmtCurrency(Number(settings.sms_monthly_cap_amount))} cap</div>
              </div>
            </div>
            <CapBar percent={monthStats.cap_percent} />
            <div className="helper" style={{ marginTop: 6 }}>{monthStats.cap_percent}% of monthly cap used</div>
          </div>

          <div className="card" style={{ padding: 20 }}>
            <div className="helper" style={{ marginBottom: 4 }}>Delivery status</div>
          ...
          <div className="card" style={{ padding: 20 }}>
            <div className="helper" style={{ marginBottom: 4 }}>Stripe reporting</div>
            <div style={{ display: 'grid', gap: 4, fontSize: 13 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span>Reported</span>
              ...
              <div className="helper" style={{ marginTop: 4 }}>
                {settings.stripe_sms_subscription_item_id ? '✓ Stripe item configured' : '⚠ No Stripe item — reporting disabled'}
              </div>
            </div>
          </div>
        </div>
      ...
      {isAdmin && (
        <div className="card" style={{ padding: 20, marginBottom: 28 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            ...
            {!editSettings && <button onClick={() => setEditSettings(true)}>Edit</button>}
          </div>
          ...
          {editSettings ? (
            <form onSubmit={handleSaveSettings} style={{ display: 'grid', gap: 12 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                  <label className="helper" style={{ display: 'block', marginBottom: 4 }}>Monthly cap ($)</label>
                  <input type="number" step="0.01" min="0" value={capAmount} onChange={e => setCapAmount(e.target.value)} style={{ width: '100%' }} required />
                  <div className="helper" style={{ marginTop: 4 }}>SMS sending stops when cost reaches this limit.</div>
                </div>
                <div>
                  <label className="helper" style={{ display: 'block', marginBottom: 4 }}>Price per SMS ($)</label>
              ...
              <div>
                <label className="helper" style={{ display: 'block', marginBottom: 4 }}>Stripe subscription item ID</label>
                <input type="text" value={stripeItemId} onChange={e => setStripeItemId(e.target.value)} placeholder="si_xxxxxxxxxxxx" style={{ width: '100%' }} />
                <div className="helper" style={{ marginTop: 4 }}>From your Stripe dashboard → Subscriptions → this tenant's subscription → line items.</div>
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
                <button type="submit" className="primary" disabled={savingSettings}>Save</button>
                <button type="button" onClick={() => setEditSettings(false)}>Cancel</button>
              </div>
            </form>
          ) : settings && (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, fontSize: 14 }}>
              <div><div className="helper">Monthly cap</div><div style={{ fontWeight: 600 }}>{fmtCurrency(Number(settings.sms_monthly_cap_amount))}</div></div>
              <div><div className="helper">Price per SMS</div><div style={{ fontWeight: 600 }}>{fmtCurrency(Number(settings.sms_price_per_unit))}</div></div>
              <div style={{ gridColumn: '1 / -1' }}>
                <div className="helper">Stripe subscription item</div>
      ...
      {history.length > 0 && (
        <div style={{ marginBottom: 28 }}>
          <h3 style={{ marginBottom: 12 }}>Monthly history</h3>
            ...
            {history.map(row => (
              <div key={row.period_start} className="card" style={{ padding: '10px 16px', display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{ minWidth: 90, fontSize: 13, fontWeight: 600 }}>
                  {new Date(row.period_start).toLocaleDateString([], { month: 'short', year: 'numeric' })}
                </div>
                <div style={{ flex: 1 }}>
      ...
      {/* ── Recent jobs ──────────────────────────────────────────── */}
      <div>
        <h3 style={{ marginBottom: 12 }}>Recent SMS jobs</h3>
        {jobs.length === 0 ? (
          <div className="helper">No SMS jobs yet.</div>
        ) : (
          <div style={{ display: 'grid', gap: 6 }}>
            {jobs.map(job => (
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexShrink: 0 }}>
                  ...
                  {job.billable && (
                    <span style={{ fontSize: 11, color: job.stripe_reported ? '#10b981' : '#f59e0b' }}>
                      {job.stripe_reported ? '✓ reported' : '⏳ pending'}
                    </span>
                  )}
                  {job.error_message && (
