    ...
    const [customerOrders, supplierOrders, stageEvents] = await Promise.all([
      sql`
        SELECT
          ...
          COALESCE(
            STRING_AGG(DISTINCT p.name, ', ' ORDER BY p.name) FILTER (WHERE oi.covers_product_id IS NULL AND oi.covers_order_item_id IS NULL),
            STRING_AGG(DISTINCT p.name, ', ' ORDER BY p.name)
          )                                   AS product_names,
          STRING_AGG(DISTINCT p.name, ', ' ORDER BY p.name) FILTER (WHERE oi.covers_product_id IS NOT NULL OR oi.covers_order_item_id IS NOT NULL) AS addon_names,
          SUM(oi.qty * oi.unit_price)::float8 AS amount,
          (SUM(oi.qty) FILTER (WHERE oi.covers_product_id IS NULL AND oi.covers_order_item_id IS NULL))::int AS total_qty
        FROM orders o
