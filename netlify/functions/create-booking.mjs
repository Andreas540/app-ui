...
async function createBooking(event) {
  try {
    ...
    try {
      // Seed default booking_confirmation template + rule if not yet created (non-blocking)
      try {
        ...
        console.error('Reminder seeding error (non-fatal):', seedErr)
      }

      // One-time migration: default all existing customers to sms_consent = true
      // Uses IS NOT TRUE to also catch NULL (not just false)
      await sql`
        UPDATE customers SET sms_consent = true
        WHERE tenant_id = ${TENANT_ID} AND sms_consent IS NOT TRUE AND sms_consent_at IS NULL
      `.catch(e => console.error('sms_consent migration error (non-fatal):', e))
